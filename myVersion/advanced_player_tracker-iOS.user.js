...
function wrapper( plugin_info ) {
	...
	// Clears all logged playerData from memory and localStorage
	window.plugin.advancedPlayerTracker.clearData=function() {
		...
		if ( confirm( "Are you sure you want to clear all tracking data? This cannot be undone." ) ) {
			window.plugin.advancedPlayerTracker.playerData=[];
			localStorage.removeItem( window.plugin.advancedPlayerTracker.LSPlayerDataKey );
			alert( "All tracking data cleared." );
		}
	};
            ...
            <div class="row">
                <div id='playerListArea' class="column" style="float:left;width:100%;box-sizing: border-box;padding-right: 5px;">
                    <textarea 
						...
						<button type="submit" form="maxfield" value="Save" onclick='window.plugin.advancedPlayerTracker.updatePlayerList(document.getElementById("listPlayers").value)'>
							Save Watch List
						</button>
						<button type="submit" form="maxfield" value="Export Data" onclick='window.plugin.advancedPlayerTracker.exportData()'>
							Export Tracking Data
						</button>
						<button type="button" onclick='window.plugin.advancedPlayerTracker.clearData()'>
							Clear All Data
						</button>
						<button type="button" onclick='window.plugin.advancedPlayerTracker.refreshMap()'>
							Refresh Now
						</button>
        </form>
			`;
