...
`;
    ...
    self.importAllBookmarks=function() {
        ...
        const portalFolders=bm.bkmrksObj.portals;
        let guids=[];

        // flatten across every folder under `portals`
        Object.values( portalFolders ).forEach( folder => {
            const b=folder.bkmrk;
            if ( !b ) return;
            // if it's already an array, great; otherwise pull out the object values
            const entries=Array.isArray( b )? b:Object.values( b );
        ...
        // dedupe
        guids=[ ...new Set( guids ) ];
        console.log( '➡️  total unique GUIDs:', guids.length );

        // map to LatLngs
        this.waypoints=guids
            .map( guid => {
                const p=window.portals[ guid ];
                if ( !p ) console.warn( '  portal not in window.portals:', guid );
                return p&&p.getLatLng();
            } )
            .filter( Boolean );

        console.log( '📍 waypoints created:', this.waypoints.length );
        this.storeWaypoints();
        this.drawRoute();
    };
  ...
  `;
        ...
        container.querySelector( 'button[name=save-route-button]' ).onclick=() => {
            ...
            self.routes[ name ]=JSON.parse( JSON.stringify( self.waypoints ) );
            self.storeRoutes();
            self.renderRoutesList( container );
            input.value='';
        };
        ...
        window.dialog( {
            html: container,
            id: self.pluginname+'-routes-dialog',
