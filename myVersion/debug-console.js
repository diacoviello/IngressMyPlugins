...
debugTab.renderLine = function (errorType, args) {
  ...
  var isAtBottom = debugContainer.scrollTop >= debugContainer.scrollHeight - debugContainer.clientHeight;
  ...
  if (isAtBottom) debugContainer.scrollTo(0, debugContainer.scrollHeight);
};
...
function overwriteNative() {
  var nativeMethods = {};
  ['log', 'warn', 'error', 'debug', 'info'].forEach(function (which) {
    nativeMethods[which] = window.console[which].bind(window.console);
  });

  function overwrite(which) {
    window.console[which] = function () {
      nativeMethods[which].apply(window.console, arguments);
