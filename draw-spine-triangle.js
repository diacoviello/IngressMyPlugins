...
function wrapper( plugin_info ) {
	...
	plugin_info.pluginId='DiabloEnMusicaGrowASpine';

	// PLUGIN START
	console.log( 'DiabloEnMusicaGrowASpine | loading plugin' )
	...
	self.setup=function() {
		...
		console.debug( 'DiabloEnMusicaGrowASpine | Checking IITC Version' );
		console.debug( 'DiabloEnMusicaGrowASpine | iitcBuildDate = '+iitcBuildDate );
		if ( iitcBuildDate<='2023-11-20-071719' ) {
			$( '#toolbox' ).append( '<a onclick="window.plugin.homogeneousFields.openDialog(); return false;">Spine Layers</a>' );
		} else {
	...
	self.getPortalsInTriangle=function( triangle, portalsToConsider ) {
		...
		console.log( 'corner portals: ', triangleLatLngs );
		
		let portalsInTriangle=[];
		let portalsInTriangleCoordinates=[];
		...
		for ( let portalGuid of portalsToConsider ) {
			...
			if ( self.pointInTriangle( portal.getLatLng(), triangleLatLngs ) ) {
				...
				portalsInTriangleCoordinates.push( portal.getLatLng() );
			}
		}
		//console.log( 'Inner portals: ', portalsInTriangle )
	...
	*/
	...
	// // this is nesting links inside of other links
	...
	self.generatePlan=function( portalData, path, hcfLevel, fieldType ) {
		...
		if ( fieldType=='hcf' ) {
			...
			if ( totalPortalsActual!==totalPortalsExpected||totalKeysActual!==totalKeysExpected ) {
				console.debug( 'DiabloEnMusicaGrowASpine | '+hcfLevel, totalPortalsActual, totalPortalsExpected, totalKeysActual, totalKeysExpected, path, plan );
	...
	self.attachEventHandler=function() {
		...
		$( "#hcf-colorPicker" ).change( function() {
			console.debug( 'DiabloEnMusicaGrowASpine | Selected color:', this.value ); // Output the selected color
	...
	self.find_hcf_plan=function() {
		...
		if ( corners.length!=3 ) {
			$( "#hcf-plan-text" ).val( "Please select only three portals." );
			return;
		}
	...
	console.log( 'DiabloEnMusicaGrowASpine | plugin is ready' )
