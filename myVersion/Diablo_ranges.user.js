...
// @name           XMP Ranges - SelectedPortal
// @category       Diablo
...
function wrapper( plugin_info ) {
	...
	window.plugin.playerRanges.layer.boot=function() {
		...
		// Portal-centered XMP ranges overlay (one set of circles centered on selected portal)
		window.plugin.playerRanges.obj.portalRangesLayer=new L.LayerGroup();
		window.addLayerGroup( 'XMP Ranges (Portal)', window.plugin.playerRanges.obj.portalRangesLayer, false );

		// draw when a portal is selected
	...
	window.plugin.playerRanges.layer.drawXmpRangesAtPortal=function( guid ) {
		if ( !guid ) return;
		var portalLayer=window.portals&&window.portals[ guid ];
		if ( !portalLayer ) return;
		var latlng=portalLayer.getLatLng();
		var layerGroup=window.plugin.playerRanges.obj.portalRangesLayer;
		if ( !layerGroup ) return;
		layerGroup.clearLayers();
		var optWeight=window.plugin.playerRanges.obj.settings.weight;
		var optOpacity=window.plugin.playerRanges.obj.settings.opacity;
		var colors=window.COLORS_LVL||[ '#ffffff', '#9d9d9d', '#b4e197', '#5bd85b', '#3fbf3f', '#228b22', '#1f7bff', '#8e44ad', '#f39c12' ];
		// Helper: extract latlng array from various circle implementations
		function _coordsFromLayer( lay ) {
			if ( !lay ) return null;
		...
		// Draw annuli (rings) so each XMP level is a band between previous radius and current radius
		var prevRadius=0;
		for ( var lvl=1;lvl<=8;lvl++ ) {
			var radius=window.plugin.playerRanges.data.getRangeValByIndex( lvl );
			if ( !radius ) continue;
	...
	// Toggle portal-centered XMP ranges overlay
	window.plugin.playerRanges.action.togglePortalRanges=function() {
		var layer=window.plugin.playerRanges.obj.portalRangesLayer;
		if ( !layer ) return;
