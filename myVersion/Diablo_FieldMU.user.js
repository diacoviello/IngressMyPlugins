...
// @description    [diabloenmusica-1.0.2.20260603.001000] Calculate approximate MU for drawn fields using WorldPop population data and S2 level-13 cell coverage. MU = population x 2/3 (empirical Ingress formula). Falls back to loaded-portal count if the API is unavailable.
...
// Bridge: expose a GM_xmlhttpRequest-backed fetch on the real page window so
// the injected wrapper (which runs in page context) can make cross-origin
// requests that would otherwise be blocked by CORS.
var _pageWin=( typeof unsafeWindow!=='undefined' ) ? unsafeWindow : window;
_pageWin._qdlFieldMU_fetch=function( url, callback ) {
	GM_xmlhttpRequest( {
		method: 'GET',
		url: url,
		onload: function( r ) { callback( null, r.responseText ); },
		onerror: function() { callback( new Error( 'request failed' ) ); },
		ontimeout: function() { callback( new Error( 'timeout' ) ); },
		timeout: 30000
	} );
};
...
function wrapper( plugin_info ) {
	...
	self.id='qdlfieldmu';
	self.title='FieldMU';
	self.version='1.0.1.20260602.002000';
	self.author='DiabloEnMusica';

	// ── Field detection ──────────────────────────────────────────────────────
	// Reproduces the triangle-finding algorithm from quickdrawlinks.updatefieldslayer.
	// Returns an array of [v1, v2, v3] latLng triples (one per unique field).
	self.getDrawnFields=function() {
		...
		// Deduplicate by sorting vertices into a canonical key
		var seen={};
		var fields=[];
	...
	// Spherical triangle area in km² via L'Huilier's theorem.
	...
	// Estimate how many S2 level-13 cells the field covers (avg cell area ~1.27 km²).
	// S2 L13 is the level Ingress is believed to use for MU population mapping.
	self.estimateS2Cells=function( v1, v2, v3 ) {
		return Math.max( 1, Math.round( self.triangleAreaKm2( v1, v2, v3 )/1.27 ) );
	};

	// ── Population data (WorldPop REST API) ──────────────────────────────────
	//
	// Formula (empirically reverse-engineered by the Ingress community,
	// most notably Gordon Ludlow's analysis):
	//
	//   MU ≈ round( Σ( S2_cell_population × coverage_fraction ) × 2/3 )
	//
	// WorldPop's stats API integrates population across the polygon area,
	// which is equivalent to that sum. We then apply the ×2/3 multiplier.
	//
	// Dataset: WPGP un-adjusted global population grid, 2020, ~1 km resolution.
	// Source:  api.worldpop.org  (open access, University of Southampton)
	//
	// Returns a Promise resolving to { population: Number, mu: Number }.
	self.fetchWorldPopMU=function( v1, v2, v3 ) {
		...
		// Use the GM_xmlhttpRequest bridge set up in the outer (userscript) scope.
		// This bypasses CORS entirely — the request is made by the browser extension,
		// not by the intel.ingress.com page origin.
		return new Promise( function( resolve, reject ) {
			if ( typeof window._qdlFieldMU_fetch!=='function' ) {
				reject( new Error( 'GM bridge not available' ) );
				return;
			}
			window._qdlFieldMU_fetch( url, function( err, responseText ) {
				if ( err ) { reject( err ); return; }
				try {
					var json=JSON.parse( responseText );
					var pop=0;
					if ( json&&json.data ) {
						pop=parseFloat( json.data.total_population||json.data.pop||json.data.population||0 );
					}
					if ( isNaN( pop ) ) pop=0;
	...
	// ── Portal helpers (fallback + corner names) ──────────────────────────────

	// Sign-based point-in-triangle for portal counting (fallback when API fails)
	...
	self.countPortalsInside=function( v1, v2, v3 ) {
		...
		for ( var guid in window.portals ) {
			if ( self.pointInTriangle( window.portals[ guid ].getLatLng(), v1, v2, v3 ) ) n++;
		}
		return n;
	};
	...
	self.showDialog=function() {
		...
		if ( fields.length===0 ) {
			...
			fields.forEach( function( verts, i ) {
				...
				html+=areaKm2.toFixed( 2 )+' km²';
				html+=' &nbsp;·&nbsp; ~'+s2count+' S2 L13 cell'+(s2count!==1 ? 's':'');
				html+=' &nbsp;·&nbsp; '+portals+' portal'+(portals!==1 ? 's':'')+' in view';
				html+='</div>';
				html+='</div>';
			} );
		...
		html+='MU = population × ⅔ · population from WorldPop 2020 (~1 km grid) integrated over S2 L13 cell coverage. ';
		html+='Empirical formula (Ludlow 2015). Niantic\'s actual value may differ.';
		html+='</p>';
		html+='</div>';
		...
		// Fire parallel WorldPop lookups, update the dialog slots as each resolves
		...
		fields.forEach( function( verts, i ) {
			...
			self.fetchWorldPopMU( v1, v2, v3 )
				...
				.catch( function() {
					...
					var portals=self.countPortalsInside( v1, v2, v3 );
					$( '#qdlfieldmu-mu-'+i ).html(
						'<span style="color:#888;">API unavailable — '+portals+' portals in view</span>'
					);
					if ( ++completed===fields.length ) onAllDone();
				} );
		} );
