...
function wrapper( plugin_info ) {
	...
	self.setup=function() {
		...
		console.debug( 'Spine Layering | Checking IITC Version' );
		console.debug( 'Spine Layering | iitcBuildDate = '+iitcBuildDate );
		if ( iitcBuildDate<='2023-11-20-071719' ) {
			$( '#toolbox' ).append( '<a onclick="window.plugin.homogeneousFields.openDialog(); return false;">Spine Layering</a>' );
		} else {
			const buttonId=IITC.toolbox.addButton( {
				label: 'Spine Layering',
	...
	*/
	...
	// helper function to recursively populate the portal data structure in triangle
	...
	self.generatePlan=function( portalData, path, hcfLevel, fieldType ) {
		...
		if ( fieldType=='hcf' ) {
			...
			if ( totalPortalsActual!==totalPortalsExpected||totalKeysActual!==totalKeysExpected ) {
				console.debug( 'Spine Layering | '+hcfLevel, totalPortalsActual, totalPortalsExpected, totalKeysActual, totalKeysExpected, path, plan );
	...
	self.attachEventHandler=function() {
		...
		$( "#hcf-colorPicker" ).change( function() {
			console.debug( 'Spine Layering | Selected color:', this.value ); // Output the selected color
	...
	console.log( 'Spine Layering | plugin is ready' )
